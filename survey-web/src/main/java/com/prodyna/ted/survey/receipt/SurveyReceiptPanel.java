...
public class SurveyReceiptPanel extends WizardStep {
    ...
    public SurveyReceiptPanel(final IModel<SurveyEntity> model) {
        ...
        adminLink.add(new Label("adminLinkText", new StringResourceModel("adminLinkText", SurveyReceiptPanel.this, null, new PropertyModel<Long>(
            model, "id"))));
        form.add(adminLink);

        Link<String> statisticsLink = new AjaxFallbackLink<String>("statisticsLink") {
            private static final long serialVersionUID = 1L;

            @Override
            public void onClick(AjaxRequestTarget target) {
                setResponsePage(new ViewSurveyPage(new PageParameters().add("id", model.getObject().getId())));
            }
        };
        statisticsLink.add(new Label("statisticsLinkText", new StringResourceModel("statisticsLinkText", SurveyReceiptPanel.this, null,
            new PropertyModel<Long>(model, "id"))));
        form.add(statisticsLink);
        ...
        surveyLink.add(new Label("surveyLinkText", new StringResourceModel("surveyLinkText", SurveyReceiptPanel.this, null, new PropertyModel<Long>(
            model, "id"))));
        form.add(surveyLink);

        final ListView<QuestionEntity> questionView =
            new ListView<QuestionEntity>("questionView", new PropertyModel<List<QuestionEntity>>(model, "questions")) {
                private static final long serialVersionUID = 1L;
